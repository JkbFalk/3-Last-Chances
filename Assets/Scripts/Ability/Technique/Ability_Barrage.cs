...
public class Ability_Barrage : Technique
{
    ...
    public Ability_Barrage(Unit ability_user) : base(ability_user)
    {
    ...
    public override void CallAbilityEvent1() {
        ...
        isRightHand = false;
        if(Is(Property.UpgradeA) && Player.Instance.Animator.GetFloat("Technique Speed") < 2) {
        ...
        if(!HoldingTechniqueButton || Player.Instance.Energy.Current < EnergyCost) {
    ...
    public static string GetAbilitySpecificEnergyCostText() {
        return GetEnergyCost(typeof(Ability_Barrage)) + "+";
    }
