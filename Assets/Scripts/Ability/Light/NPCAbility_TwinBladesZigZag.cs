...
public class NPCAbility_TwinBladesZigZag : Ability {
    ...
    public NPCAbility_TwinBladesZigZag(Unit ability_user) : base(ability_user) {
        ...
        DamageSources.Add(new DamageSource(50 / 2, 350 / 2, Constants.DamageType.Light) {Knockback = 250});
        DamageSources.Add(new DamageSource(400 / 2, 0, Constants.DamageType.Light, "2") {Knockback = 500});
    ...
    public override void CallAbilityEvent1()
    {
        if(User.CurrentTarget != null) {
            ...
            User.ApplyForce(new Vector2(400 * (User.Actions.IsFlipped ? -1 : 1), (User.IsBoss ? 1 : 0.4f) * 500 * (_startByGoingUp ? 1 : -1)), this);
        }
    }

    public override void CallAbilityEvent2()
    {
        User.ApplyForce(new Vector2(400 * (User.Actions.IsFlipped ? -1 : 1), (User.IsBoss ? 1 : 0.4f) * 750 * (_startByGoingUp ? -1 : 1)), this);
    }

    public override void CallAbilityEvent3()
    {
        User.ApplyForce(new Vector2(150 * (User.Actions.IsFlipped ? -1 : 1), (User.IsBoss ? 1 : 0.4f) * 350 * (_startByGoingUp ? 1 : -1)), this);
