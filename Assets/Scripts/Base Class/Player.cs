...
public class Player : Unit
{
    ...
    public Stat StancePower;
    public Stat ItemPower;
    public Stat ToolPower;
    public Stat PassivePowerUpPower;
    public Stat HealthRestorationPower;
    public Stat HealthPotionPower;
    public float GlobalEnemyStaggerBarRegenerationModifier = 0;
    ...
    public float Ammo {
        ...
        set {
            ...
            float maxAmmo = (Player.Instance?.CurrentStance != null && Player.Instance?.CurrentStance?.StanceEffect is Stance_Gunslinger) ? 8 : 6;
            if (value < 0)
            {
                _ammo = 0;
            }
    ...
    public Dictionary<Type, int> CurrentTechniqueStacks = new() {
        ...
        {typeof(Ability_ShadowForm), 1},
        {typeof(Ability_LightningSpeed), 1},
        {typeof(Ability_EnergyJavelin), 1}
    };
    ...
    public float StealthDuration = 1;
    ...
    public void UpdateTechniqueStacksAmount(Type technique, int stacks, bool is_ultimate = false)
    {
        ...
        Cooldown cooldown = TechniqueCooldowns.FirstOrDefault(cd => cd.Type == technique && cd.Id == (is_ultimate ? "IsUltimate" : ""));
    ...
    public void UpdateAllStacksWhileNotInCombat()
    {
        ...
        {
            Cooldown cooldown1 = TechniqueCooldowns.FirstOrDefault(cd => cd.Type == ability && cd.Id == "");
            if (cooldown1 != null)
            {
                cooldown1.RemainingDuration = 0;
            }
            Cooldown cooldown2 = TechniqueCooldowns.FirstOrDefault(cd => cd.Type == ability && cd.Id == "IsUltimate");
            if (cooldown2 != null)
            {
    ...
    public Stance CurrentStance
    {
        ...
        set
        {
            ...
            EventManager.StanceSwitched.Invoke(previousValue?.GetType(), _currentStance?.GetType());
        }
    }
    ...
    public bool CanStopPreparingForUltimate = false;
    public bool WillStopPreparingForUltimate = false;
    ...
    public void ReduceAllRemainingCooldowns(float reduced_by_percentage)
    {
        foreach(Cooldown cd in Player.Instance.TechniqueCooldowns) {
            cd.RemainingDuration = cd.RemainingDuration - cd.RemainingDuration * reduced_by_percentage / 100;
        }
        foreach(Cooldown cd in Player.Instance.EffectCooldowns) {
            cd.RemainingDuration = cd.RemainingDuration - cd.RemainingDuration * reduced_by_percentage / 100;
        }
        Player.Instance.ToolCooldown.RemainingDuration = Player.Instance.ToolCooldown.RemainingDuration - Player.Instance.ToolCooldown.RemainingDuration * reduced_by_percentage / 100;
    }
}