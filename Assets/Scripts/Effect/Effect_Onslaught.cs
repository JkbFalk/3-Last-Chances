...
public class Effect_Onslaught : Effect
{
    public GameObject Vfx;
    public int OnslaughtLevel = 0;
    public Effect_Onslaught(float damage_increase, SourceOfEffect source_of_effect) : base(source_of_effect)
    {
        ...
        DescriptionLabel = "Effect_Onslaught_Explanation";
    }
    ...
    public override void OnInvokeHitDealt(Damage damage) {
        if(damage?.SourceOfDamage?.User == TargetOfEffect && ((damage?.SourceOfDamage?.User is Player && damage.SourceOfDamage.IsBasicAttack) || (damage?.SourceOfDamage?.User is not Player && damage.SourceOfDamage.IsCounterable == false && damage.SourceOfDamage.IsUncounterable == false))) {
