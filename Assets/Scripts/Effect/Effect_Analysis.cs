...
public class Effect_Analysis : Effect
{
    ...
    public override void OnInvokeHitDealt(Damage damage) {
        if(damage?.SourceOfDamage?.User == TargetOfEffect && ((damage?.SourceOfDamage?.User is Player && damage.SourceOfDamage.IsTechnique) || (damage?.SourceOfDamage?.User is not Player && damage.SourceOfDamage.DamageType == Constants.DamageType.Magic))) {
