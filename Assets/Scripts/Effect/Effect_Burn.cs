...
public class Effect_Burn : Effect
{
    ...
    public Effect_Burn(float stagger_per_second, SourceOfEffect source_of_effect) : base(source_of_effect)
    {
        Type = EffectType.Debuff;
        _initialDecayingAmount = stagger_per_second;
        DisplayEffectIndicator = true;
        BehaviourWhenDuplicateEffect = BehaviourWhenDuplicateEffectEnum.AddDecayingAmount;
        DescriptionLabel = "Effect_Burn_Explanation";
    ...
    public override void OnInvokeEffectStarted(Effect effect) {
        if(effect.TargetOfEffect == TargetOfEffect && effect.GetType().IsSubclassOf(typeof(Effect_Staggered))) {
            Damage damage = new Damage(TargetOfEffect, SourceOfEffect.SourceAbility, null) { AbilityDamageSource = new Ability.DamageSource(0, 0, Constants.DamageType.None), Properites = new List<Damage.DamageProperty> { Damage.DamageProperty.Burn }, Injury = DecayingAmount * 15}.DisableSoundOnEnemyHit().CalculateDamage();
    ...
    public override void OnFixedUpdate()
    {
        ...
        if(counter >= 50 && TargetOfEffect != null) {
            counter = 0;
            EffectIndicatorText = Utils.GetFormattedFloat(DecayingAmount);
            Damage damage = new Damage(TargetOfEffect, SourceOfEffect.SourceAbility, null) { AbilityDamageSource = new Ability.DamageSource(0, 0, Constants.DamageType.None), IsDamageOverTime = true, Properites = new List<Damage.DamageProperty> { Damage.DamageProperty.Burn }, Stagger = DecayingAmount}.DisableSoundOnEnemyHit().CalculateDamage();
            AddVisualEffect();
        }
    }
}
