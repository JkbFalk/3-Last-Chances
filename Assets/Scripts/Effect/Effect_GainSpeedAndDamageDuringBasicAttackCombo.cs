...
public class Effect_GainSpeedAndDamageDuringBasicAttackCombo : Effect {
    ...
    public Effect_GainSpeedAndDamageDuringBasicAttackCombo(SourceOfEffect source_of_effect) : base(source_of_effect) {
    ...
    public override void OnInvokeAbilityUsed(Ability ability)
    {
        ...
        if(ability is BA_Polearm_F || ability is BA_Polearm_FF || ability is BA_Polearm_FFF) {
            if(HeavySpeed == null || HeavyInjury == null || HeavyStagger == null || HeavySpeed.EffectEnded || HeavyInjury.EffectEnded || HeavyStagger.EffectEnded) {
                HeavySpeed = new Effect_ChangeStat(Player.Instance.HeavyAttackSpeed, SourceOfEffect) {PercentageAmount = SpeedBonusPerHit, ShowsInUI = true, EffectIndicatorText = Utils.GetFormattedFloat(SpeedBonusPerHit) + "%", PathToEffectGraphic = "UI/HeavyAttackSpeed"};
