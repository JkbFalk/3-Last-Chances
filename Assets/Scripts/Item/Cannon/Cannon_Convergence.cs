    ...
    public override List<Effect> GetFirstModifier() {
        return new List<Effect> {
            ...
            new Effect_CustomizableEffectOnEvent(new(this)) { EffectTypeName="NoDescription", FlatAmount = GetFirstModifierEffectValue() * 1.5625f, ConditionCheckForProjectileCreated = new Func<Projectile, bool>((proj) => 
                    (proj.SourceAbility.User == Player.Instance && proj.SourceAbility.Is(Ability.AbilityProperty.BasicAttack))), ActionOnProjectileCreated = new Action<Projectile, Effect_CustomizableEffectOnEvent> ((ability, effect) =>  {
