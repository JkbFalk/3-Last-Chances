...
public class Greatsword_Vengeance : Item
{
    ...
    public override List<Effect> GetFirstModifier() {
        return new List<Effect> { new Effect_CustomizableDamageChange(new(this)) {UsesTheFollowingEffects=new() {typeof(Effect_Onslaught)},EffectTypeName="ApplyOnslaughtOnBAs", CustomParam = GetFirstModifierEffectValue() * 0.25f, DescriptionParameters = new List<String> { Utils.GetFormattedFloat(GetFirstModifierEffectValue() * 0.25f)}, ConditionCheckAfterHitDamageCalculation = new Func<Damage, Effect_CustomizableDamageChange, bool>((damage, effect) =>
                    (damage.SourceOfDamage.User == Player.Instance && damage.SourceOfDamage.IsBasicAttack && damage.AbilityDamageSource.DamageType == Constants.DamageType.Heavy)),
